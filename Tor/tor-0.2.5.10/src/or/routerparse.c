...
#ifdef IPC_MODE
static int tokenize_string_IPC(memarea_t *area,
                           const char *start, const char *end,
                           smartlist_t *out,
                           token_rule_t *table,
                           int flags);
static directory_token_t *get_next_token_IPC(memarea_t *area,
                                         const char **s,
                                         const char *eos,
                                         token_rule_t *table);
#endif
...
networkstatus_t *
networkstatus_parse_vote_from_string_IPC(const char *s, const char **eos_out,
                                     networkstatus_type_t ns_type)
{
  ...
  if (tokenize_string_IPC(area,s, end_of_footer, footer_tokens,
...
networkstatus_parse_detached_signatures(const char *s, const char *eos)
{
  ...
  area = memarea_new();
#ifdef IPC_MODE
  if (tokenize_string_IPC(area,s, eos, tokens,
...
get_next_token_IPC(memarea_t *area,
...
#ifdef IPC_MODE
static int
tokenize_string_IPC(memarea_t *area,
                ...
                token_rule_t *table, int flags)
{
  ...
  while (*s < end && (!tok || tok->tp != EOF_)) {
    tok = get_next_token_IPC(area, s, end, table);
    if (tok->tp == ERR_) {
