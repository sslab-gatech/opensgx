...
#ifdef IPC_MODE
int
dirserv_add_own_fingerprint_IPC(const char *nickname, char *fp)
{
  if (!fingerprint_list)
 ...
 * authorities) */
networkstatus_t *
dirserv_generate_networkstatus_vote_obj_IPC(authority_cert_t *cert)
{
  const or_options_t *options = get_options();
  ...
  if (read(fd_et, &buf_len, sizeof(int)) < 0)   return NULL;
  tmp_buf = (char *)malloc(buf_len+1);
  if (read(fd_et, tmp_buf, buf_len+1) < 0)      return NULL;

  if(!strncmp(tmp_buf, "GET_DIGEST_ERROR", buf_len)) {
