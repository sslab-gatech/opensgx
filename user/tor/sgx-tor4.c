...
int exit_node_handling(int fd_te, int fd_et, int flags)
{
	...
	while(1) {
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ID_KEY_INIT", buf_len)) {
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ID_KEY_CR", buf_len)) {
			sgx_printf("Exit Node %d secret_id_key creation.\n", exit_node_num);
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ID_KEY_LD", buf_len)) {
			sgx_printf("Exit Node %d secret_id_key loading.\n", exit_node_num);
		...
		// CLIENT_KEY INIT
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_CLIENT_KEY_INIT", buf_len)) {
			sgx_printf("Exit Node %d client_key initialization.\n", exit_node_num);
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_IDENTITY_KEY_NULL", buf_len)) {
			sgx_printf("Check server_identity of %d is null.\n", exit_node_num);

			if(secret_id_key == NULL) {
				sgx_puts("Error: secret_id_key is null\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_TLS_CERTIFICATE", buf_len)) {
			sgx_printf("Exit Node %d certificate creation.\n", exit_node_num);
//			assert(secret_id_key);
			...
			if(!(sign_pkey = crypto_pk_get_evp_pkey_(secret_id_key, 1))) {
				sgx_puts("TLS Certificate creation error\n");
            ...
            sgx_write(fd_et, bio_buffer, 512);
            sgx_write(fd_et, bio_buffer+512, bio_length+1-512);

			sgx_free(sign_pkey);
			sgx_free(pkey);
			sgx_free(cname);
			sgx_free(cname_sign);
			sgx_free(serial_number);
			sgx_free(name);
			sgx_free(name_issuer);
			sgx_free(x509);
            sgx_free(bio);
            sgx_free(bio_buffer);
            sgx_free(bio_pointer);
			sgx_free(tmp_buf);
			continue;
		}

		// EXIT_NODE_TLS_CERTIFICATE CREATION
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_TLS_CERTIFICATE_SELF", buf_len)) {
			sgx_printf("Exit Node %d certificate self creation.\n", exit_node_num);
//			assert(secret_id_key);
			...
			if(!(sign_pkey = crypto_pk_get_evp_pkey_(secret_id_key, 1))) {
				sgx_puts("TLS Certificate creation self error\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_FINGERPRINT", buf_len)) {
			sgx_printf("Giving Exit Node %d fingerprint.\n", exit_node_num);

            char fingerprint[FINGERPRINT_LEN+1];

			if(crypto_pk_get_fingerprint(secret_id_key, fingerprint, 0) < 0) {
				sgx_puts("Error computing fingerprint for secret_id_key\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_FINGERPRINT_HASH", buf_len)) {
			sgx_printf("Giving Exit Node %d hash fingerprint.\n", exit_node_num);

            char fingerprint[FINGERPRINT_LEN+1];

			if(crypto_pk_get_hashed_fingerprint(secret_id_key, fingerprint) < 0) {
				sgx_puts("Error computing hashed fingerprint for secret_id_key\n");
		...
		// GET EXIT_NODE_FINGERPRINT
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_FINGERPRINT_MAIN", buf_len)) {
			sgx_printf("Giving Exit Node %d fingerprint for main.\n", exit_node_num);
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_CLIENT_ID_KEY_SET", buf_len)) {
			sgx_printf("\nCheck Exit Node %d client key exists.\n", exit_node_num);

			if(client_id_key == NULL) {
				sgx_puts("Loading key is needed for tor process\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_DIGEST", buf_len)) {
			sgx_printf("Giving Exit Node %d digest.\n", exit_node_num);

            char digest[DIGEST_LEN];
			if(crypto_pk_get_digest(secret_id_key, digest) < 0) {
				sgx_puts("Error getting digest for secret_id_key\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_APPEND_DIROBJ", buf_len)) {
			sgx_printf("Giving Exit Node %d append dirobj.\n", exit_node_num);

			char sig[DIROBJ_MAX_SIG_LEN+1];
			sgx_memset(sig, 0, sizeof(sig));
			...
			if(router_append_dirobj_signature(sig, sizeof(sig), digest, DIGEST_LEN,
												secret_id_key) < 0) {
				sgx_puts("Error append dirobj for secret_id_key\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_PUBKEY_STR", buf_len)) {
			sgx_printf("Giving Exit Node %d publickey string.\n", exit_node_num);
			...
			if(crypto_pk_write_public_key_to_string(secret_id_key,
				...
				sgx_puts("write identity_pkey to string failed!\n");
			...
			sgx_free(identity_pkey);
			sgx_free(tmp_buf);
			continue;
		}

		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_GET_DIROBJ", buf_len)) {
			sgx_printf("Giving Exit Node %d get dirobj.\n", exit_node_num);
			...
			if (!(sig = router_get_dirobj_signature(digest, DIGEST_LEN, 
													secret_id_key))) {
				sgx_puts("Couldn't sign router descriptor\n");
		...
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ONION_KEY_INIT", buf_len)) {
			sgx_puts("Initializeing exit node onion key.\n");
			if(onionkey == NULL) {
				buf_len = sgx_strlen("CREATION");
				sgx_write(fd_et, &buf_len, sizeof(int));
				sgx_write(fd_et, "CREATION", buf_len+1);
		...
		// ONION_KEY CREATION
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ONION_KEY_CR", buf_len)) {
			sgx_printf("Exit Node %d onion_key creation.\n", exit_node_num);
		...
		// IDENTITY_KEY LOADING
		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ONION_KEY_LD", buf_len)) {
			sgx_printf("Exit Node %d onionkey loading.\n", exit_node_num);

			sgx_free(tmp_buf);
			continue;
		}

		if(!sgx_strncmp(tmp_buf, "EXIT_NODE_ONION_PUBKEY_STR", buf_len)) {
			sgx_printf("Giving Exit Node %d onion publickey string.\n", exit_node_num);
			...
			if(crypto_pk_write_public_key_to_string(onionkey,
				...
				sgx_puts("write onion_pkey to string failed!\n");
