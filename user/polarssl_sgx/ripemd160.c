...
{
    sgx_memset( ctx, 0, sizeof( ripemd160_context ) );
}
...
int ripemd160_self_test( int verbose )
{
    ...
    sgx_memset( output, 0, sizeof output );
    ...
    for( i = 0; i < TESTS; i++ )
    {
        ...
        ripemd160( (const unsigned char *) ripemd160_test_input[i],
                   sgx_strlen( ripemd160_test_input[i] ),
                   output );

        if( sgx_memcmp( output, ripemd160_test_md[i], 20 ) != 0 )
        {
            if( verbose != 0 )
        ...
        for( j = 0; j < KEYS; j++ )
        {
            ...
            if( sgx_memcmp( output, ripemd160_test_hmac[j][i], 20 ) != 0 )
            {
                if( verbose != 0 )
                    polarssl_printf( "failed\n" );
