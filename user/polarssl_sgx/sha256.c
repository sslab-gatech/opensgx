...
void sgx_sha256_init( sha256_context *ctx )
{
    sgx_memset( ctx, 0, sizeof( sha256_context ) );
}

void sgx_sha256_free( sha256_context *ctx )
{
    if( ctx == NULL )
        return;
 ...
 */
void sgx_sha256_starts( sha256_context *ctx, int is224 )
{
    ctx->total[0] = 0;
    ctx->total[1] = 0;
...
void sgx_sha256_process( sha256_context *ctx, const unsigned char data[64] )
{
 ...
 */
void sgx_sha256_update( sha256_context *ctx, const unsigned char *input,
                    size_t ilen )
{
    size_t fill;
    uint32_t left;
    ...
    if( left && ilen >= fill )
    {
        ...
        sgx_sha256_process( ctx, ctx->buffer );
        input += fill;
        ilen  -= fill;
        left = 0;
    }
 ...
 */
void sgx_sha256_finish( sha256_context *ctx, unsigned char output[32] )
{
    uint32_t last, padn;
    uint32_t high, low;
    ...
    sgx_sha256_update( ctx, sha256_padding, padn );
    sgx_sha256_update( ctx, msglen, 8 );
 ...
 */
void sgx_sha256( const unsigned char *input, size_t ilen,
             unsigned char output[32], int is224 )
{
    sha256_context ctx;

    sgx_sha256_init( &ctx );
    sgx_sha256_starts( &ctx, is224 );
    sgx_sha256_update( &ctx, input, ilen );
    sgx_sha256_finish( &ctx, output );
    sgx_sha256_free( &ctx );
}
...
int sha256_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( sgx_memcmp( sha256sum, sha256_test_sum[i], 32 - k * 4 ) != 0 )
        {
            if( verbose != 0 )
    ...
    for( i = 0; i < 14; i++ )
    {
        ...
        if( sgx_memcmp( sha256sum, sha256_hmac_test_sum[i], buflen ) != 0 )
        {
            if( verbose != 0 )
                polarssl_printf( "failed\n" );
