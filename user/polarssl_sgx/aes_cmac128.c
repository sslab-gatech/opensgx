 ...
 * zero a structure 
 */
#define ZERO_STRUCT(x)      sgx_memset((char *)&(x), 0, sizeof(x))

/** 
 * zero a structure given a pointer to the structure 
 */
#define ZERO_STRUCTP(x)     do{ if((x) != NULL) sgx_memset((char *)(x), 0, sizeof(*(x)));} while(0)
...
static inline void sgx_aes_cmac_128_left_shift_1(const uint8_t in[16], uint8_t out[16])
{
    uint8_t overflow = 0;
    int8_t i;
...
static inline void sgx_aes_cmac_128_xor(const uint8_t in1[16], const uint8_t in2[16], 
                                        uint8_t out[16])
{
    uint8_t i;
 ...
 */
void sgx_aes_cmac128_starts(aes_cmac128_context *ctx, const uint8_t K[16])
{
    uint8_t L[16];
    ...
    sgx_aes_setkey_enc(&ctx->aes_key, K, 128);

    /* step 1 - generate subkeys k1 and k2 */
    sgx_aes_crypt_cbc(&ctx->aes_key, AES_ENCRYPT, 16, iv, const_Zero, L);

    if (_MSB(L) == 0) {
        sgx_aes_cmac_128_left_shift_1(L, ctx->K1);
    } else {
        uint8_t tmp_block[16];

        sgx_aes_cmac_128_left_shift_1(L, tmp_block);
        sgx_aes_cmac_128_xor(tmp_block, const_Rb, ctx->K1);
        ZERO_STRUCT(tmp_block);
    }

    if (_MSB(ctx->K1) == 0) {
        sgx_aes_cmac_128_left_shift_1(ctx->K1, ctx->K2);
    } else {
        uint8_t tmp_block[16];

        sgx_aes_cmac_128_left_shift_1(ctx->K1, tmp_block);
        sgx_aes_cmac_128_xor(tmp_block, const_Rb, ctx->K2);
        ZERO_STRUCT(tmp_block);
   }
 ...
 */
void sgx_aes_cmac128_final(aes_cmac128_context *ctx, uint8_t T[16])
{
    ...
    if (ctx->last_len < 16) {
        ...
        sgx_aes_cmac_128_xor(ctx->last, ctx->K2, tmp_block);
    } else {
        sgx_aes_cmac_128_xor(ctx->last, ctx->K1, tmp_block);
    }

    sgx_aes_cmac_128_xor(tmp_block, ctx->X, Y);
    sgx_aes_crypt_cbc(&ctx->aes_key, AES_ENCRYPT, 16, iv, Y, T);
