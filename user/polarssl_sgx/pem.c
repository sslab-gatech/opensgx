...
{
    sgx_memset( ctx, 0, sizeof( pem_context ) );
}
                        ...
                        const unsigned char *pwd, size_t pwdlen )
{
    ...
    sgx_memcpy( key + 16, md5sum, use_len );
                     ...
                     size_t pwdlen, size_t *use_len )
{
    ...
    *use_len = end - data;
    ...
    if( sgx_memcmp( s1, "Proc-Type: 4,ENCRYPTED", 22 ) == 0 )
    {
#if defined(POLARSSL_MD5_C) && defined(POLARSSL_CIPHER_MODE_CBC) &&         \
    ( defined(POLARSSL_DES_C) || defined(POLARSSL_AES_C) )
        enc++;
...
#if defined(POLARSSL_DES_C)
        if( sgx_memcmp( s1, "DEK-Info: DES-EDE3-CBC,", 23 ) == 0 )
        {
        ...
        else if( sgx_memcmp( s1, "DEK-Info: DES-CBC,", 18 ) == 0 )
        {
...
#if defined(POLARSSL_AES_C)
        if( sgx_memcmp( s1, "DEK-Info: AES-", 14 ) == 0 )
        {
            if( sgx_memcmp( s1, "DEK-Info: AES-128-CBC,", 22 ) == 0 )
                enc_alg = POLARSSL_CIPHER_AES_128_CBC;
            else if( sgx_memcmp( s1, "DEK-Info: AES-192-CBC,", 22 ) == 0 )
                enc_alg = POLARSSL_CIPHER_AES_192_CBC;
            else if( sgx_memcmp( s1, "DEK-Info: AES-256-CBC,", 22 ) == 0 )
                      ...
                      unsigned char *buf, size_t buf_len, size_t *olen )
{
    ...
    add_len = sgx_strlen( header ) + sgx_strlen( footer ) + ( use_len / 64 ) + 1;
    ...
    sgx_memcpy( p, header, sgx_strlen( header ) );
    p += sgx_strlen( header );
    c = encode_buf;
    ...
    sgx_memcpy( p, footer, sgx_strlen( footer ) );
    p += sgx_strlen( footer );
