...
{
    sgx_memset( ctx, 0, sizeof( des_context ) );
}
...
{
    sgx_memset( ctx, 0, sizeof( des3_context ) );
}
...
int des_key_check_weak( const unsigned char key[DES_KEY_SIZE] )
{
    ...
    for( i = 0; i < WEAK_KEY_COUNT; i++ )
        if( sgx_memcmp( weak_key_table[i], key, DES_KEY_SIZE) == 0 )
            return( 1 );
                    ...
                    unsigned char *output )
{
    ...
    else /* DES_DECRYPT */
    {
        while( length > 0 )
        {
            sgx_memcpy( temp, input, 8 );
            des_crypt_ecb( ctx, input, output );

            for( i = 0; i < 8; i++ )
                output[i] = (unsigned char)( output[i] ^ iv[i] );

            sgx_memcpy( iv, temp, 8 );
...
int des_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        sgx_memcpy( buf, des3_test_buf, 8 );
        ...
        if( ( v == DES_DECRYPT &&
                sgx_memcmp( buf, des3_test_ecb_dec[u], 8 ) != 0 ) ||
            ( v != DES_DECRYPT &&
                sgx_memcmp( buf, des3_test_ecb_enc[u], 8 ) != 0 ) )
        {
            if( verbose != 0 )
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        sgx_memcpy( iv,  des3_test_iv,  8 );
        sgx_memcpy( prv, des3_test_iv,  8 );
        sgx_memcpy( buf, des3_test_buf, 8 );
        ...
        else
        {
            for( j = 0; j < 10000; j++ )
            {
                ...
                sgx_memcpy( tmp, prv, 8 );
                sgx_memcpy( prv, buf, 8 );
                sgx_memcpy( buf, tmp, 8 );
            }

            sgx_memcpy( buf, prv, 8 );
        }

        if( ( v == DES_DECRYPT &&
                sgx_memcmp( buf, des3_test_cbc_dec[u], 8 ) != 0 ) ||
            ( v != DES_DECRYPT &&
                sgx_memcmp( buf, des3_test_cbc_enc[u], 8 ) != 0 ) )
        {
            if( verbose != 0 )
                polarssl_printf( "failed\n" );
