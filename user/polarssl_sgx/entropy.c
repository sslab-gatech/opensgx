...
void sgx_entropy_init( entropy_context *ctx )
{
    sgx_memset( ctx, 0, sizeof(entropy_context) );
...
#else
    sgx_sha256_starts( &ctx->accumulator, 0 );
#endif
#if defined(POLARSSL_HAVEGE_C)
    ...
    sgx_entropy_add_source( ctx, NULL, ENTROPY_MIN_HARDCLOCK );
#endif
#if defined(POLARSSL_HAVEGE_C)
...
void sgx_entropy_free( entropy_context *ctx )
{
#if defined(POLARSSL_HAVEGE_C)
    havege_free( &ctx->havege_data );
#endif
    sgx_polarssl_zeroize( ctx, sizeof( entropy_context ) );
#if defined(POLARSSL_THREADING_C)
    polarssl_mutex_free( &ctx->mutex );
#endif
}

int sgx_entropy_add_source( entropy_context *ctx,
                        void *p_source,
                        size_t threshold )
//int sgx_entropy_add_source( entropy_context *ctx,
//                        void *f_source,
 ...
 */
static int sgx_entropy_update( entropy_context *ctx, unsigned char source_id,
...
#else
    sgx_sha256_update( &ctx->accumulator, header, 2 );
    sgx_sha256_update( &ctx->accumulator, p, use_len );
#endif
 ...
 */
static int sgx_entropy_gather_internal( entropy_context *ctx )
{
    int ret, i;
    ...
    {
        olen = 0;
        if( ( ret = sgx_hardclock_poll( ctx->source[i].p_source,
...
//                        buf, ENTROPY_MAX_GATHER, &olen ) ) != 0 )
        ...
        if( olen > 0 )
        {
            sgx_entropy_update( ctx, (unsigned char) i, buf, olen );
...
int sgx_entropy_func( void *data, unsigned char *output, size_t len )
{
    ...
    do
    {
        ...
        if( ( ret = sgx_entropy_gather_internal( ctx ) ) != 0 )
            goto exit;
    ...
    sgx_memset( buf, 0, ENTROPY_BLOCK_SIZE );
     ...
     */
    sgx_memset( &ctx->accumulator, 0, sizeof( sha512_context ) );
    sha512_starts( &ctx->accumulator, 0 );
    ...
    sgx_sha256_finish( &ctx->accumulator, buf );

    /*
     * Reset accumulator and counters and recycle existing entropy
     */
    sgx_memset( &ctx->accumulator, 0, sizeof( sha256_context ) );
    sgx_sha256_starts( &ctx->accumulator, 0 );
    sgx_sha256_update( &ctx->accumulator, buf, ENTROPY_BLOCK_SIZE );

    /*
     * Perform second SHA-256 on entropy
     */
    sgx_sha256( buf, ENTROPY_BLOCK_SIZE, buf, 0 );
#endif /* POLARSSL_ENTROPY_SHA512_ACCUMULATOR */

    for( i = 0; i < ctx->source_count; i++ )
        ctx->source[i].size = 0;

    sgx_memcpy( output, buf, len );
