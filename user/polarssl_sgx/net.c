...
int net_connect( int *fd, const char *host, int port )
{
    ...
    sgx_memset( port_str, 0, sizeof( port_str ) );
    snprintf( port_str, sizeof( port_str ), "%d", port );

    /* Do name resolution with both IPv6 and IPv4, but only TCP */
    sgx_memset( &hints, 0, sizeof( hints ) );
    ...
    sgx_memcpy( (void *) &server_addr.sin_addr,
    ...
    *client_fd = (int) accept( bind_fd, (struct sockaddr *)
    ...
    if( client_ip != NULL )
    {
        ...
        if( client_addr.ss_family == AF_INET )
        {
            ...
            sgx_memcpy( client_ip, &addr4->sin_addr.s_addr,
                        sizeof( addr4->sin_addr.s_addr ) );
        }
        else
        {
            struct sockaddr_in6 *addr6 = (struct sockaddr_in6 *) &client_addr;
            sgx_memcpy( client_ip, &addr6->sin6_addr.s6_addr,
                        sizeof( addr6->sin6_addr.s6_addr ) );
        }
#else
        sgx_memcpy( client_ip, &client_addr.sin_addr.s_addr,
