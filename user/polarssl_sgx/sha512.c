...
{
    sgx_memset( ctx, 0, sizeof( sha512_context ) );
}
                         ...
                         size_t keylen, int is384 )
{
    ...
    sgx_memset( ctx->ipad, 0x36, 128 );
    sgx_memset( ctx->opad, 0x5C, 128 );
...
int sha512_self_test( int verbose )
{
    ...
    for( i = 0; i < 6; i++ )
    {
        ...
        if( sgx_memcmp( sha512sum, sha512_test_sum[i], 64 - k * 16 ) != 0 )
        {
            if( verbose != 0 )
    ...
    for( i = 0; i < 14; i++ )
    {
        ...
        if( sgx_memcmp( sha512sum, sha512_hmac_test_sum[i], buflen ) != 0 )
        {
            if( verbose != 0 )
                polarssl_printf( "failed\n" );
